...
## Goal
GitHub Copilot SDKを用いたマルチエージェント環境を構築し、要件分析から計画、実装提案、テスト設計・実行、レビュー、修正ループ、成果物保存までを一貫して運用できる状態を維持する
...
6. Apply safe edit orchestration and fix-loop reruns when review requires follow-up
...
- For blocked review findings, convert the recommendation into a concrete fix slice and re-run `test_execution` and `review`
