...
- `integration-orchestrator-flow` (integration): Orchestrator completes requirements, planning, documentation, source-code implementation, and test-code implementation phases.
  ...
  - assertion: test_implementation is completed
...
- `e2e-failure-recovery` (end_to_end): Failure path can recover through fix loop and persist recovered source and test implementation outputs.
  - assertion: recovered implementation, test_implementation, test, and review outputs are used in final persistence
...
- `acceptance-2` (integration): Session rotation can preserve context through persistence
  - assertion: Generated workflow outputs provide evidence for this criterion
- `acceptance-3` (integration): Planning, source-code implementation, test-code implementation, testing, and review are represented
  - assertion: Generated workflow outputs provide evidence for this criterion
- `slice-implementation-agent` (integration): Validate implementation slice `implementation-agent`: Add an implementation agent that turns plans into executable source-code change proposals.
  - assertion: Targets are identified: src/impliforge/agents/implementation.py
  - assertion: Dependencies are satisfied before execution
  - assertion: Downstream handoff metadata is present for test_design, test_implementation, test_execution, review, and fixer
- `slice-documentation-agent` (integration): Validate implementation slice `documentation-agent`: Add a documentation agent that produces design and runbook artifacts.
  - assertion: Targets are identified: src/impliforge/agents/documentation.py
  - assertion: Dependencies are satisfied before execution
...
- `slice-orchestrator-integration` (integration): Validate implementation slice `orchestrator-integration`: Wire documentation and implementation phases into the orchestrator.
  - assertion: Targets are identified: src/impliforge/main.py
  - assertion: Dependencies are satisfied before execution
...
- `slice-src-allowlisted-edit-phase` (integration): Validate implementation slice `src-allowlisted-edit-phase`: Promote approved implementation proposals into structured source edits under src/impliforge/.
  - assertion: Targets are identified: src/impliforge/main.py, src/impliforge/runtime/editor.py, src/impliforge/agents/implementation.py
  - assertion: Dependencies are satisfied before execution
- `slice-test-implementation-agent` (integration): Validate implementation slice `test-implementation-agent`: Add a test implementation agent that turns test plans and implementation diffs into executable test changes.
  - assertion: Targets are identified: src/impliforge/agents/test_implementation.py
  - assertion: Dependencies are satisfied before execution
  - assertion: Test code, fixtures, or helpers are proposed consistently with the implementation diff
...
- Source implementation diffs and paired test implementation diffs for alignment validation
